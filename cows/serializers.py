...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = User; fields = ['id', 'username', 'is_staff', 'is_active', 'date_joined']; read_only_fields = ['date_joined']
...
# === Serwery Krów (z polami rodowodu) ===
class CowSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cow; fields = ['id', 'tag_id', 'name', 'breed', 'birth_date', 'gender', 'photo', 'age', 'status', 'dam', 'sire', 'dam_name', 'sire_name', 'created_at', 'updated_at']; read_only_fields = ['created_at', 'updated_at', 'age', 'dam_name', 'sire_name']
...
class CowCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cow; fields = ['id', 'tag_id', 'name', 'breed', 'birth_date', 'gender', 'photo', 'status', 'dam', 'sire']; extra_kwargs = {'photo': {'required': False, 'allow_null': True, 'read_only': True} }
    ...
    def validate_birth_date(self, value):
        from datetime import date
        if value > date.today(): raise serializers.ValidationError("Data urodzenia nie może być w przyszłości")
        return value
...
class CowListSerializer(serializers.ModelSerializer): 
    ...
    sire_name = serializers.CharField(source='sire.name', read_only=True, allow_null=True)
    class Meta:
        model = Cow; fields = ['id', 'tag_id', 'name', 'breed', 'birth_date', 'gender', 'photo', 'age', 'status', 'dam_name', 'sire_name'] 
    def get_age(self, obj):
        from datetime import date
        today = date.today(); age = today.year - obj.birth_date.year - ((today.month, today.day) < (obj.birth_date.month, obj.birth_date.day)); return age
    def get_photo(self, obj):
...
# === SERIALIZERY DLA DRZEWA (Rodowodu) ===
class CowPedigreeSimpleSerializer(serializers.ModelSerializer):
    class Meta:
        model = Cow
        fields = ['id', 'name', 'tag_id', 'gender']
class CowPedigreeParentSerializer(serializers.ModelSerializer):
    dam = CowPedigreeSimpleSerializer(read_only=True)
    sire = CowPedigreeSimpleSerializer(read_only=True)
    class Meta:
        model = Cow
        fields = ['id', 'name', 'tag_id', 'gender', 'dam', 'sire']
class CowPedigreeSerializer(serializers.ModelSerializer):
    dam = CowPedigreeParentSerializer(read_only=True)
    sire = CowPedigreeParentSerializer(read_only=True)
    class Meta:
        model = Cow
        fields = ['id', 'name', 'tag_id', 'gender', 'dam', 'sire']
class CowOffspringSerializer(serializers.ModelSerializer):
    class Meta:
        model = Cow
        fields = ['id', 'name', 'tag_id', 'gender', 'status']
# ================================================
...
class CowDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CowDocument
        fields = ['id', 'cow', 'title', 'file', 'file_url', 'filename', 'uploaded_at', 'user']
        read_only_fields = ['user', 'uploaded_at', 'filename', 'file_url']
...
class TaskSerializer(serializers.ModelSerializer):
    user = serializers.StringRelatedField(read_only=True)
    cow = serializers.PrimaryKeyRelatedField(queryset=Cow.objects.filter(status='ACTIVE'), allow_null=True, required=False)
    cow_name = serializers.CharField(source='cow.name', read_only=True, allow_null=True)
    cow_tag_id = serializers.CharField(source='cow.tag_id', read_only=True, allow_null=True)
    class Meta:
